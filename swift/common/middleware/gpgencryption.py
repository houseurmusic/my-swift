...
class GPGEncryption():
    ...
    def dump_buffer(self, timeout = .1):
        time.sleep(timeout)
        buf = self.p.read()
    ...
    def close_and_dump(self, timeout = .2):
        ...
        timed_out = False
        while buf == '' and not timed_out:
            buf = self.dump_buffer()
            time.sleep(.01)
            print self.p.readerr()
            poll = self.p.wait(os.WNOHANG)
            timed_out = (time_stamp + 1) < time.time()
        if timed_out:
            print 'timed out when waiting for process to terminate'
            #self.p.terminate(0)
            ...
            if self.test_branch:
                ...
                while True:
                    ...
                    try:
                        ...
                        chunk = self.stream_iter.next()
                        print "chunk size before strip= " + str(len(chunk))
                        gpg_final = chunk[len(chunk) - 1] == self.term_char
                        chunk = chunk.strip(self.term_char)
                        print "chunk size after strip= " + str(len(chunk))
                        #print "chunk = " + chunk
                        if len(chunk) > 0:
